...
class BinarySearchTree:
    ...
    def insert(self, x):
        '''
        Put x into the search tree
        raise DuplicateKey if already there.
        '''
        # if tree is empty
    ...
    def __find_min(self, p, q):
        '''Return the smallest element under node p, 
        and its parent. q should be the parent of p'''
        if p is None: return None
    ...
    def __find_and_parent(self, x):
        '''
        Search for x, returning the node containing x and its parent. If x
        doesn't exist, we return None and x's would-be parent.
        '''
        q = None           # parent
